...
func TestPickExtraMetadata(t *testing.T) {
...
// resolver sends them
func TestNonGRPCLBBalancerGetsNoGRPCLBAddress(t *testing.T) {
...
// channel.
func TestAddressAttributesInNewSubConn(t *testing.T) {
	const (
		testAttrKey      = "foo"
		testAttrVal      = "bar"
...
func TestMetadataInAddressAttributes(t *testing.T) {
	const (
		testMDKey      = "test-md"
...
func TestServersSwap(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
