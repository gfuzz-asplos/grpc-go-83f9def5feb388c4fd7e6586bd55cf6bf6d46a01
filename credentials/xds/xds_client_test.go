...
func TestClientCredsWithoutFallback(t *testing.T) {
...
// providers.
func TestClientCredsInvalidHandshakeInfo(t *testing.T) {
...
func TestClientCredsProviderFailure(t *testing.T) {
	opts := ClientOptions{FallbackCreds: makeFallbackClientCreds(t)}
	creds, err := NewClientCredentials(opts)
	if err != nil {
		t.Fatalf("NewClientCredentials(%v) failed: %v", opts, err)
...
func TestClientCredsHandshakeTimeout(t *testing.T) {
...
func TestClientCredsHandshakeFailure(t *testing.T) {
	tests := []struct {
		desc          string
		handshakeFunc testHandshakeFunc
		rootProvider  certprovider.Provider
